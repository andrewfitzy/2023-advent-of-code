...
import {solve, getCardMatches} from '../../src/day_04/solution_pt2';
import {testSkipsCi} from '../test_utils/skip_ci';

describe('test getCardMatches', () => {
  test('getCardMatches expects 4', () => {
    const input = 'Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53';
    const result = getCardMatches(input);
    expect(result.length).toBe(4);
  });

  test('getCardMatches expects 2', () => {
...
describe('test solve', () => {
  test('example input expects 30', () => {
    const fileInput = `${__dirname}/example_input_01.txt`;
    const input: Array<string> = readFileAsStringArray(fileInput);
    expect(solve(input)).toBe(30);
  });

  testSkipsCi('puzzle input expects 19499881', () => {
    const fileInput = `${__dirname}/puzzle_input.txt`;
    const input: Array<string> = readFileAsStringArray(fileInput);
    expect(solve(input)).toBe(19499881);
  });
});
