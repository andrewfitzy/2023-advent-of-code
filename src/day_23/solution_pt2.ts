...
export const solve = (input: Array<string>): number => {
  ...
  const longestPath = getLongestPath(start!, end!, visited, graphMap);
  return longestPath;
};
...
const getNextMoves = (
  ...
  for (const [rowDelta, colDelta] of possibleMoves) {
    ...
    if (
      ...
      // could be a space or direction so just add it
  ...
  for (const step of nextSteps!.values()) {
    if (!visited.has(step.location.toString())) {
      runningCost = Math.max(
        runningCost,
        getLongestPath(step.location, destination, visited, graphMap) +
          step.cost
      );
    }
  }
